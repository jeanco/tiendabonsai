    ...
    <table class="table shop_table" width="100%">
        ...
        <tbody>
          ...
          <tr>
             ...
             @if($company->checkout_active)
             <td class="">{{ $order['symbol'] }}{{ number_format($order['total']*$order['sales_exchange'], $order['decimal'], '.', '') }}</td>
             @else
             <td>{{ $company->checkout_message }}</td>
             @endif
             <td>
               <a href="/pdf-pedido-cliente/{{ $order['uuid'] }}" target="_blank" class="btn-clear">

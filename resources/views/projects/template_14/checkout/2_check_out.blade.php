						...
						<h3>2. Formas de Entrega</h3>
							...
							<form id="other-billing_form">
								...
								<div class="payments">
									<ul>
										<li>
											<label class="container_radio">Recoger en Tienda (Gratis)<a href="javascript:void(0);"></a>
										...
										<div id="pay_1{{ $payment_way['id'] }}" class="collapse {{ $flag == 1 ? 'show' : '' }}" data-parent="#accordion_pay">
										 	<div class="card-body method-payments">
												...
												<div class="form-check">
													<input class="form-check-input radio-btn-{{ $i }}" type="radio" id="select_payment_{{$i}}" name="account_id"  data-payment="{{ $payment_way['id'] }}" data-index="{{ $account['id'] }}" {{ ($flag == 1 && $i == 0) ? 'checked' : '' }}>
													<label class="form-check-label" for="select_payment_{{$i}}">{{ $account['name'] }}</label><br>
...
<!-- SPECIFIC CSS -->
    <link href="/template_14/css/checkout.css" rel="stylesheet">
...
<script>
  ...
  //   	// Other address Panel
	...
	<script src="/template_14/js/orden_info.js"></script>

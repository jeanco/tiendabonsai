<div class="sidebar left">
  <aside class="single-side-box archive mb-4">
      ...
      <div class="archive-list">
          <ul>
            <li class="category"><a href="#" class="catalog-category__change {{(Request::get('category') == '') ? 'active' : ''}}" data-index='' data-slug=''>Todos</a></li>
            @foreach($categories as $key => $category)
              <li class="category"><a href="#" class="catalog-category__change {{($category['id'] == Request::get('category')) ? 'active' : ''}}" data-slug={{ $category['slug'] }} data-index="{{ $category['id'] }}">{{ $category['name'] }}</a></li>
            @endforeach
          </ul>
      </div>
  </aside><br>
  <aside class="single-side-box feature" style="display: none;">
  ...
  <aside class="single-side-box feature">
    @include('miranda.catalog.category_attribute_filter')
  </aside><br>

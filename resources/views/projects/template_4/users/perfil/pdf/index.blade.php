    ...
    <div class="content-margin" style="margin-top: 50px;">
      ...
      <div style="padding: 15px 35px;">
        <table width="100%">
          ...
          <tr>  <td>Provincia:</td>               <td>{{ $t['customer']['province']  }}</td>                  </tr>
          <tr>  <td>Distrito:</td>               <td> {{ $t['customer']['district']  }} </td>                  </tr>
          <tr>  <td>Ciudad:</td>               <td>{{ $t['customer']['city'] }}</td>                  </tr>
          <tr>  <td>Dirección:</td>                <td>{{ $t['customer']['address'] }}</td>               </tr>
          <tr>  <td>Forma de Pago:</td>        <td>{{ $t['payment_way_name'] }}</td>       </tr>
          <tr>  <td>Tipo de moneda:</td>        <td>Soles</td>       </tr>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="pdf-table">
              ...
              {{--<span style="text-decoration: line-through; font-size: 13px;">{{$product['discount'] }}</span> <br> --}}{{ $product['discount'] }}
              @else
              {{$product['discount'] }}
              @endif</td>
            <td class="pdf-table">{{ ($product['discount']-$product['price'])*$product['quantity'] }}</td>
            <td class="pdf-table" style="color: #00549d; font-weight: 700;">{{ $product['sub_total'] }}</td>
          </tr>
          @endforeach
<!--           <tr>
         ...
         <tfoot>
          <tr>
            ...
            <td class="pdf-foot-table">S/ {{ $t['subtotal'] }}</td>
          </tr>
          <tr>

            <td class=" text-right" colspan="5">COSTO DE ENVÍO:</td>
            <td class="">S/ {{ $t['shipping_cost'] }}</td>
          </tr>
          {{--<tr>

            <td class=" text-right" colspan="5">DSCTO. GLOBAL:</td>
            <td class="">S/ {{ $t['global_discount'] }}</td>
          </tr>--}}

          <tr>
            <td class=" text-right" colspan="5">DSCTO CUPÓN:</td>

            <td class="">S/ {{ $t['discount'] }}</td>
          </tr>
          @if($t['credit_card_comission'])
          <tr>
            <td class=" text-right" colspan="5">Comisión pago con tarjeta:</td>

            <td class="">S/ {{ round($t['credit_card_comission_amount'], 2) }}</td>
          </tr>
          @endif
          <tr>
            <td class="pdf-foot-table text-right" colspan="3">TOTAL UNIDADES:</td>
            <td class="pdf-foot-table">{{ $quantity_total }} UND.</td>
            <td class="pdf-foot-table text-right" >TOTAL:</td>
            <td class="pdf-foot-table">S/ {{ $t['total'] }}</td>
          </tr>
        </tfoot>
      </table><br>
        ...
        <ul class="u-list-reset">
            <li class="u-mt3">
              <div class="c-entity">
                ...
                <img src="{{ $t['account_logo'] }}" height="45px" class="c-entity__image">

                <p class="u-mt0 u-mb1">{{ $t['account_description'] }}</p>

  ...
  <div style="margin: 0px 50px;">
      ...
      <tr>
        ...
        <td width="40%" style="border: 3px solid #000;">
          <center>
            ...
            @php
              $length_of_id = strlen($order_history['id']);
              $how_many_zeros_to_add = 8 - $length_of_id;
            @endphp
            <span><b>PE - {{ str_pad($order_history['id'], $how_many_zeros_to_add +1, "0", STR_PAD_LEFT) }}</b></span>
          </center>
        </td>
      </tr>
    ...
    <table width="100%">
      <tr>
        ...
        <td>{{ $order_history['created_at']->format('H:i:s') }}</td>
      </tr>
      <tr>
        <td>Señor(es):</td>
        <td>{{ $order_history['customer']['first_name'] }} {{ $order_history['customer']['last_name'] }}</td>
        <td>R.U.C.:</td>
        <td>{{ $order_history['customer']['legal_name'] }}</td>
      </tr>
      <tr>
        <td>Dirección Cliente:</td>
        <td>{{ $order_history['customer']['address'] }}</td>
        <td>Tipo Moneda:</td>
        <td>{{ $order_history['currency']['name'] }}</td>
      </tr>

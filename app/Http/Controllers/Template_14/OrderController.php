...
use App\Http\Requests\StoreOrderTemplate14Request;
use App\Mail\CustomerSuccessfulOrder;
use App\Mail\CustomerSuccessfulOrderTemplate14;
use App\Order;
...
class OrderController extends Controller {
	...
	public function store(StoreOrderTemplate14Request $request) {
		...
		foreach ($valores as $key => $value) {
			...
			if ($product->transfer_available && $data['account_id'] != 4 && Carbon::parse($product->transfer_end_at)->format('Y-m-d') >= $now) {
		...
		foreach ($companies as $key => $company) {
			...
			$order->subtotal = 0;
			$order->business_name = $request->business_name_social;
		...
		if ($account->payment_way_id == 3) {
			...
			$company = Company::whereMain(1)->first();
			$this->send_email_to_customer_from_order_history($order_history->id, $company->email, $customer_email, $customer_name, 'Has realizado un Pedido', 'Se ha hecho un pedido');
		}
		//----------------
	...
	public function send_email_to_customer_from_order_history($order_history_id, $company_email, $customer_email, $customer_name, $subject_customer = 'Has realizado un pedido', $subject_company = 'Se ha hecho un pedido') {
		...
		Mail::to($customer_email)->send(new CustomerSuccessfulOrderTemplate14($data));
		...
		Mail::to($company_main->email)->send(new CustomerSuccessfulOrderTemplate14($data));
	}
	...
	public function show($id) {
		...
		return view('template_14.users.perfil.pdf.index', compact('t'));

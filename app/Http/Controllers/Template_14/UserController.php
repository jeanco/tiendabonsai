...
use App\Http\Requests\ProfileUpdateTemplate3Request;
...
class UserController extends Controller {
	...
	public function register(RegisterUserTemplate3Request $request) {
		...
		$new_alternative_direction->address = $data['address'];
		$new_alternative_direction->identity_document = $data['identity_document'];
		$new_alternative_direction->first_name = $data['first_name'];
		$new_alternative_direction->last_name = $data['last_name'];
		$new_alternative_direction->cel = $data['cel_whatsapp'];
		$new_alternative_direction->customer_id = $new_customer->id;
		...
		Mail::to($company->email)->send(new UserRegisteredCompany($send_company, $data['email']));
		Mail::to($data['email'])->send(new UserRegisteredMail($send, $company->email));
	...
	public function show_view(Request $request) {
		return view("template_14.users.register.index");
	}
	...
	public function show_my_profile_view() {
		...
		// 	);
		// }
		return view('template_14.users.perfil.index', compact('customer', 'countries', 'user_orders', 'user_cupons', 'user_id'));
	}

	public function update_my_profile($customer_id, ProfileUpdateTemplate3Request $request) {

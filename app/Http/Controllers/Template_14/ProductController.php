...
class ProductController extends Controller {
	...
	public function get_view($slug, Request $request) {
		...
		$brand = Product::whereId($product_id)->with('brands_perfil_product_template_14')->first();
		//dd($brand);

		if (count($brand->brands_perfil_product_template_14) > 0) {
			$brand = $brand->brands_perfil_product_template_14->first();
			$brand = $brand->name;
		} else {
			$brand = "Otros";
		}
		...
		return view('template_14.catalog.perfil.index', compact('product', 'related_products', 'reviews', 'div_reviews','brand'));
	}
	...
	public function fetch_data(Request $request) {
		...
		if ($request->ajax()) {
			...
			return view("template_14.catalog.3_right_catalog", compact('products'))->render();
		}
	}

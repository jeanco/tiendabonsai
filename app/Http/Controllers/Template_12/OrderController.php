...
use App\Http\Requests\StoreOrderTemplate12Request;
use App\Mail\CustomerSuccessfulOrderTemplate7;
...
class OrderController extends Controller {
	...
	public function store(StoreOrderTemplate12Request $request) {
	...
	public function send_email_to_customer_from_order_history($order_history_id, $company_email, $customer_email, $customer_name, $subject_customer = 'Has realizado un pedido', $subject_company = 'Se ha hecho un pedido') {
		...
		Mail::to($customer_email)->send(new CustomerSuccessfulOrderTemplate7($data));
		...
		Mail::to($company_main->email)->send(new CustomerSuccessfulOrderTemplate7($data));
	}
	...
	public function show($id) {
		...
		return view('template_12.users.perfil.pdf.index', compact('t'));

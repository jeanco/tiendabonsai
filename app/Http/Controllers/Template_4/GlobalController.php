...
class GlobalController extends Controller {
	...
	public function compose(View $view) {
		...
		$secction_promotions = Campaign::whereCategoryId(2)->where('deleted_at', '=', null)->get();
		//Template_4
		$header_categories = Category::wherePublished(1)
			->select(['id', 'name', 'icon', 'icon_white', 'slug','image_thumb'])
			->with(['subcategories' => function ($query) {
				$query->select(['id', 'category_id', 'name', 'slug']);
				$query->wherePublished(1);
			}])
		...
		if (count($video) == 0) {
			...
			foreach ($video as $key => $value) {
				$videos[$key] = $value->resource;
				$url_parsed_arr = parse_url($videos[$key]);
				$videos[$key] = substr($url_parsed_arr['query'], 2);

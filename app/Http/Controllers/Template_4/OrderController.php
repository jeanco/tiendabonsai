...
use App\Mail\CustomerSuccessfulOrderTemplate4;
use App\Order;
...
class OrderController extends Controller {
	...
	public function send_email_to_customer_from_order_history($order_history_id, $company_email, $customer_email, $customer_name, $subject_customer = 'Has realizado un pedido', $subject_company = 'Se ha hecho un pedido') {
		...
		Mail::to($customer_email)->send(new CustomerSuccessfulOrderTemplate4($data));
		...
		Mail::to($company_main->email)->send(new CustomerSuccessfulOrderTemplate4($data));
	}
	...
	public function show($id) {
		...
		$t = array(
			...
			'credit_card_comission' => $order->credit_card_comission,
			'credit_card_comission_amount' => $order->credit_card_comission_amount,
		);

		//return view('oyeepe.users.perfil.pdf.index', compact('t'));
		return view('template_4.users.perfil.pdf.index', compact('t'));

...
class UserController extends Controller {
	...
	public function show_my_profile_view() {
		...
		$document_type = DB::table('user_type_documents')
			->where('id', $customer->document_type)
			->first();
		...
		foreach ($orders as $key => $order) {
			...
			$user_orders[] = array(
				...
				'total' => $order->total,
				'id' => $order->id,
				'decimal' => $order->currency->decimal ? 2 : 0,
				'symbol' => $order->currency->symbol,
				'sales_exchange' => $order->currency->exchange_rate->sales_exchange,
			);
		}
		...
		$company = Company::whereMain(1)
			->select(['checkout_active', 'checkout_message'])
			->first();

		return view('template_11.users.perfil.index', compact('customer', 'countries', 'user_orders', 'user_cupons', 'user_id', 'document_type', 'company'));
	}

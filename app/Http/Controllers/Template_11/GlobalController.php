...
class GlobalController extends Controller {
	...
	public function compose(View $view) {
		...
		$checkout_active = $company_main->checkout_active;
		$checkout_message = $company_main->checkout_message;
        ...
        $place_id_selected = session()->get('place_id');

        $currency_id = session()->get('currency_id', 1);

        $current_currency = Currency::where('id', $currency_id)
			//->whereDefaultCurrency(1)
			->whereActive(1)
	    	->get(['symbol', 'code', 'decimal', 'id']);

	  	if (!count($current_currency)) {
	  		$current_currency = Currency::whereDefaultCurrency(1)
				->whereActive(1)
		    	->get(['symbol', 'code', 'decimal', 'id']);
		    	
		    $currency_id = $current_currency[0]->id;
	  	}

        $currencies = Currency::whereActive(1)
        	->where('id', '!=', $currency_id)
        	->get(['symbol', 'code', 'decimal', 'id']);

		$symbol_ = $current_currency[0]['symbol'];
		$decimal_ = $current_currency[0]['decimal'] == 1 ? 2 : 0;
		$currency_id = $current_currency[0]['id'];
		...
		$view->with('categories', $categories)->with('company_categories', $company_categories)->with('company_main', $company_main)->with('values', $our_values)
			...
			->with('place_id_selected', $place_id_selected)
			->with('currencies', $currencies)
			->with('current_currency', $current_currency)
			->with('symbol_', $symbol_)
			->with('decimal_', $decimal_)
			->with('symbol', $symbol_)
			->with('decimal', $decimal_)
			->with('currency_id', $currency_id)
			->with('rate', $rate)
			->with('checkout_active', $checkout_active)
			->with('checkout_message', $checkout_message);

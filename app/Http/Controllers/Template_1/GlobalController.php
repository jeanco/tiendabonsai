...
class GlobalController extends Controller {
	...
	public function compose(View $view) {
		$categories = Category::wherePublished(1)
			->select(['id', 'name', 'icon', 'icon_white', 'slug','image'])
		...
		$posts_publicaciones = Post::orderBy('id', 'DESC')
			->wherePublished(1)
			->whereTagId(1)//Publicaciones
		...
		$posts_proyecto_bonsai = Post::orderBy('id', 'DESC')
			->wherePublished(1)
			->whereTagId(2)//Publicaciones
			->where('deleted_at', null)
			->with('image')
			->take(2)
			->get();
		...
		if (count($video) == 0) {
			...
			foreach ($video as $key => $value) {
				...
				$videoyou[$key] = $value->resource;
				$url_parsed_arr = parse_url($videoyou[$key]);
				$videoyou[$key] = substr($url_parsed_arr['query'], 2);
				// code...
			}
		...
		$view->with('categories', $categories)->with('company_categories', $company_categories)->with('company_main', $company_main)->with('values', $our_values)
			->with('posts', $posts)
			->with('posts_publicaciones', $posts_publicaciones)
			->with('posts_proyecto_bonsai', $posts_proyecto_bonsai)

...
class ProductController extends Controller {
	...
	public function get_view($slug, Request $request) {
		...
		return view('template_1.catalog.perfil.index', compact('product', 'related_products', 'item', 'presentations'));
	}
	...
	public function fetch_data(Request $request) {
		...
		if ($request->ajax()) {
			...
			$products = Product::with('photo')
				...
				->with('brands_perfil_product_template_1')
				->where('is_main_product', true)
				->with(['product_child' => function($query){
					$query->orderBy('price', 'ASC');
					$query->where('is_main_product', false);
			...
			return view("template_1.catalog.3_right_catalog", compact('products'))->render();

...
class ProductController extends Controller {
	...
	public function get_view($slug, Request $request) {
		...
		$product = Product::with('images')
			->with(['price_promoted' => function ($query) use ($place_id) {
		...
		return view('template_2.catalog.perfil.index', compact('product', 'related_products'));
	}
	...
	public function fetch_data(Request $request) {
		...
		if ($request->ajax()) {
			...
			return view("template_2.catalog.3_right_catalog", compact('products'))->render();
		}
	}
			...
			foreach ($valores as $key => $value) {
				...
				if ($product->price_promoted) {
					if ($product->price_promoted->promotion_end_at) {
						if ($product->price_promoted->promotion_end_at >= $now) {

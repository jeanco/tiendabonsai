	...
	public function show($id) {
		...
		foreach ($order_products as $l => $order_product) {
			...
			$p[] = array(
				...
				'sub_total' => number_format($order_product->quantity * $order_product->price, 3, '.', ''),
			);
		...
		return view('template_2.users.perfil.pdf.index', compact('t'));
	...
	public function store(StoreOrderTemplate2Request $request) {
		...
		foreach ($valores as $key => $value) {
			...
			$product = Product::with(['unit_price' => function ($query) use ($place_id) {
				...
				}])->with(['price_promoted' => function ($query) use ($place_id, $now) {
					$query->wherePlaceId($place_id);
					$query->whereDate('promotion_end_at', '>=', $now);
				}])->find($key);
		...
		if ($account->payment_way_id == 3) {
			...
			$company = Company::whereMain(1)->first();
			$this->send_email_to_customer_from_order_history_to_pay($order_history->id, $company->email, $customer_email, $customer_name, 'Has realizado una compra - Verfica el Código de Pago de Culqi', 'Se ha hecho una Compra por Pagar');*/
		}
		//----------------

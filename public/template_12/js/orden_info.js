// (() => {
	...
	function calculate_prices(shipping_cost){
		axios.get(`/api/template_12/products/cart-lite?ids=${localStorage.getItem("cart")}`)
			.then((response) => {
				let _subTotal = 0,
					_content = ``,
					_discount = 0,
				...
				response.data.forEach((product) => {
					...
					_content += `
								<li class="clearfix"><img width="0px" src="${product.image}" data-src="${product.image}" class="lazy" alt="Image" ><em>${product.quantity}x ${product.name}</em>  <span>S/${product.price}</span></li>`;
				});
				...
				getElement(`#checkout_shipping-cost`).innerHTML = `S/${shipping_cost}`;
	...
	function get_cities_by_country(){
		...
		if (getElement(`select[name="country_id"]`).value != '') {
			axios.get(`/api/template_12/country/${getElement(`select[name="country_id"]`).value}/cities`)
				.then((response) => {
	...
	function get_provinces_by_city(){
		axios.get(`/api/template_12/city/${getElement(`select[name="city_id"]`).value}/provinces`)
			.then((response) => {
	...
	function get_districts_by_province(){
		axios.get(`/api/template_12/province/${$(`select[name="province_id"]`).val()}/districts`)
		.then((response) => {
	...
	$(`select[name="district_id"]`).on('change', function(E){
		...
		if (E.target.value != '') {
			axios.get(`/api/template_12/district/${E.target.value}/shipping-cost`)
				.then((response) => {
...
function culqi() {
		if (Culqi.token) { 
				...
				$.ajax({
						type: 'POST',
						url: '/api/template_12/order-confirm-payment',
...
getElement(`input[name="identity_document"]`)
	.addEventListener('keyup', function(e){
		...
		if (e.target.value.length >= 8) {
			axios.get(`/api/template_12/customer?identity_document=${e.target.value}`)
				.then((response) => {
					if (response.data) {
...
if (getElement(`#country_id`).value != '') {
	axios.get(`/api/template_12/country/${getElement(`#country_id`).value}/cities`)
		.then((response) => {
	...
	.then((city_id) => {
		axios.get(`/api/template_12/city/${city_id}/provinces`)
			.then((response) => {
	...
	.then(() => {
		axios.get(`/api/template_12/province/${getElement(`#province_id`).value}/districts`)
		.then((response) => {
	...
	.then(() => {
		axios.get(`/api/template_12/district/${getElement(`#district_id`).value}/shipping-cost`)
			.then((response) => {
				calculate_prices(response.data);
				getElement(`#shipping-error`).innerHTML = ``;
			})

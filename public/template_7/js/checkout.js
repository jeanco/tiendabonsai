(() => {
    ...
    axios.get(`/api/template_7/products/cart?ids=${localStorage.getItem("cart")}`)
        .then((response) => {
    ...
    $(`#apply_coupon`)
        .on('click', function(){
            ...
            axios.get(`/api/template_7/coupon/${_code}/information?ids=${localStorage.getItem("cart")}`)
                .then((response) => {
                    Swal.fire(
                    '',
    ...
    $(`select[name="place_id"]`).on('change', function(e){
        axios.post(`/api/template_7/set-place`, {

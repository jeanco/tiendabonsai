(() => {
	...
	if (getElement(`#profile_country`).value != null) {
		axios.get(`/api/template_10/country/${getElement(`#profile_country`).value}/cities`)
			.then((response) => {
		...
		.then(() => {
			axios.get(`/api/template_10/province/${getElement(`#profile_province-id`).value}/districts`)
			.then((response) => {

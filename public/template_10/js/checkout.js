(() => {
    ...
    axios.get(`/api/template_10/products/cart?ids=${localStorage.getItem("cart")}`)
        .then((response) => {
    ...
    function change_quantity(_id, _newQuantity, _price, _elementTotal){
        ...
        if (localStorage.getItem(`cart`) != null) {
            ...
            _elementTotal.html(`$ ${_newTotal.toFixed(2)}`);
    ...
    var myEfficientFn = debounce(function() {
        axios.get(`/api/template_10/products/cart-lite?ids=${localStorage.getItem("cart")}`)
            .then((response) => {
                ...
                getElement(`#cart-discount`).innerHTML = `<span>Dscto Cupón</span>$ ${_discount.toFixed(2)}`;
                getElement(`#cart-subtotal`).innerHTML = `<span>Subtotal</span>$ ${_subTotal.toFixed(2)}`;
                getElement(`#cart-total`).innerHTML = `<span>Total</span>$ ${_total.toFixed(2)}`;
            });
    }, 1500);
    ...
    $(`#apply_coupon`)
        .on('click', function(){
            ...
            axios.get(`/api/template_10/coupon/${_code}/information?ids=${localStorage.getItem("cart")}`)
                .then((response) => {
                    Swal.fire(
                    '',

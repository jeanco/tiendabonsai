(() => {
	...
	function fetch_data(page, category_slug, attributes_id, per_page, q, subcategory_slug) {
		// ocultar();
		$.ajax({
			url: `/api/template_10/fetch-data-products?page=${page}&category_id=${category_slug}&subcategory_id=${subcategory_slug}&attributes_id=${attributes_id}&per_page=${per_page}&q=${q}`,
	...
	function fetch_data_attributes(subcategory_slug) {
		$.ajax({
			url: `/api/template_10/fetch-data-attributes?subcategory_slug=${subcategory_slug}`,
			success: function(data) {

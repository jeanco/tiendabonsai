// (() => {
	...
	function calculate_prices(shipping_cost, credit_card_comission = false){
		...
		axios.get(`/api/template_10/products/cart-lite?ids=${localStorage.getItem("cart")}`)
			.then((response) => {
				let _subTotal = 0,
					_content = ``,
					_discount = 0,
				...
				response.data.forEach((product) => {
					...
					_content += `
								<li class="clearfix"><em>${product.quantity}x ${product.name}</em>  <span>$${product.price}</span></li>`;
				});
	            ...
	            if (credit_card_comission) {
	            	...
	            	_comission = ((_subTotal / 100) * 5 * 100) / 100;
				...
				getElement(`#checkout_discount`).innerHTML = `$${_discount.toFixed(2)}`;
				getElement(`#checkout_subtotal`).innerHTML = `$${_subTotal.toFixed(2)}`;
				getElement(`#checkout_shipping-cost`).innerHTML = `$${shipping_cost.toFixed(2)}`;
				getElement(`#checkout_credit-card-comission`).innerHTML = `$${_comission.toFixed(2)}`;

				// if (shipping_cost == 0 && getElement(`#delivery-home`).checked) {
				// 	getElement(`#checkout_shipping-cost`).innerHTML = `Seleccione Distrito`;
				// }

				getElement(`#checkout_total`).innerHTML = `$${_total.toFixed(2)}`;
	...
	getElement(`#checkout__save`)
		.addEventListener('click', function(e) {
			...
			// } else {
				...
				let _accountId = $(`.method-payments_`).find('input:checked')[0].dataset.index;
	...
	function get_cities_by_country(){
		...
		if (getElement(`select[name="country_id"]`).value != '') {
			axios.get(`/api/template_10/country/${getElement(`select[name="country_id"]`).value}/cities`)
				.then((response) => {
	...
	function get_provinces_by_city(){
		axios.get(`/api/template_10/city/${getElement(`select[name="city_id"]`).value}/provinces`)
			.then((response) => {
	...
	function get_districts_by_province(){
		axios.get(`/api/template_10/province/${$(`select[name="province_id"]`).val()}/districts`)
		.then((response) => {
	...
	$(`select[name="district_id"]`).on('change', function(E){
		...
		if (E.target.value != '') {
			axios.get(`/api/template_10/district/${E.target.value}/shipping-cost?cart=${localStorage.getItem('cart')}`)
				.then((response) => {
...
function culqi() {
		if (Culqi.token) { 
				...
				$.ajax({
						type: 'POST',
						url: '/api/template_10/order-confirm-payment',
...
getElement(`input[name="identity_document"]`)
	.addEventListener('keyup', function(e){
		...
		if (e.target.value.length >= 8) {
			axios.get(`/api/template_10/customer?identity_document=${e.target.value}`)
				.then((response) => {
					if (response.data) {
...
if (getElement(`#country_id`).value != '') {
	axios.get(`/api/template_10/country/${getElement(`#country_id`).value}/cities`)
		.then((response) => {
	...
	.then((city_id) => {
		axios.get(`/api/template_10/city/${city_id}/provinces`)
			.then((response) => {
	...
	.then(() => {
		axios.get(`/api/template_10/province/${getElement(`#province_id`).value}/districts`)
		.then((response) => {
	...
	.then(() => {
		axios.get(`/api/template_10/district/${getElement(`#district_id`).value}/shipping-cost`)
			.then((response) => {
				calculate_prices(response.data);
				getElement(`#shipping-error`).innerHTML = ``;
			})

(() => {
    ...
    axios.get(`/api/template_14/products/cart?ids=${localStorage.getItem("cart")}`)
        .then((response) => {
                        ...
                        `
                            <tr>
                                ...
                                <td style="line-height: 13px;">
                                    <strong>S/. ${product.price}</strong><br>
                                    
                                     ${product.transfer_price ? `<span style="font-size:11px; color:red;">Exclusivo con Transferencia</span><br><span class="transfer_price font_bold" style="background: yellow;line-height: 22px;">S/ ${product.transfer_price}</span>` : ``}
                                ...
                                <td>
                                    <strong>S/. ${product.price*product.quantity}</strong>
                                </td>
                                <td class="options">
    ...
    function reCalculatingTotal() {
        ...
        axios.get(`/api/template_14/products/cart-lite?ids=${localStorage.getItem("cart")}&payment_id=3`)
            .then((response) => {
                ...
                getElement(`#cart-discount`).innerHTML = `<span>Dscto Cupón</span>S/${_discount}`;
                getElement(`#cart-subtotal`).innerHTML = `<span>Subtotal</span>S/${_subTotal}`;
                getElement(`#cart-total`).innerHTML = `<span>Total</span>S/${_total}`;
            });
    ...
    $(`#apply_coupon`)
        .on('click', function(){
            ...
            axios.get(`/api/template_14/coupon/${_code}/information?ids=${localStorage.getItem("cart")}`)
                .then((response) => {
                    Swal.fire(
                    '',
    ...
    $(`select[name="place_id"]`).on('change', function(e){
        axios.post(`/api/template_14/set-place`, {

(() => {
	getElement(`#subscription__save`)
		.addEventListener('click', (e) => {
			...
			lockWindow();
			//$(`.text-error`).html("");
			axios.post(`/api/template_14/subscriptions`, {
					'email': getElement(`#subscription_email`).value,
					'name': getElement(`#subscription_name`).value,
					'lastname': getElement(`#subscription_lastname`).value,
				...
				.then((response) => {
					...
					$(`#subscription_modal_`).fadeIn(300);
				})
				.catch((error) => {
					unlockWindow();
					...
					$.each(error.response.data, function(key, value) {
						if (key == "email") {
							...
							$.each(value, function(errores, eror) {
								$('#subscription-error-lastname').append("<li class='error-block'>" + eror + "</li>");
							});

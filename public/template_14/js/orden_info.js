...
/*$('#select_payment_0').on('ifChecked', function(event) {
    alert();
  });
*/

var isChecked = document.getElementById('select_payment_0').checked;
if(isChecked){

var article = document.getElementById('select_payment_0');
var acount = article.dataset.index // "3"
var payment = article.dataset.payment // "3"

//precio_transfer
calculate_prices(max,payment);
...
//alert(payment);
	...
	function calculate_prices(shipping_cost, payment_id = 2){
		axios.get(`/api/template_14/products/cart-lite?ids=${localStorage.getItem("cart")}&payment_id=${payment_id}`)
			.then((response) => {
				let _subTotal = 0,
					_content = ``,
					_discount = 0,
				...
				response.data.forEach((product) => {
					...
					if (product.transfer_available) {
						_content += `<li class="clearfix">
										<em>${product.quantity}x ${product.name}</em>
										<span style="font-size:11px; color:red;">Exclusivo con Transferencia</span><br>
										<span class="transfer_price font_bold" style="background: yellow;line-height: 22px;" >S/ ${product.price_transfer}</span>
											<div class="price_box  my-3" style="line-height: 12px;">
												<br><br>
												<span class="${product.active_old}">S/ ${product.old_price}</span>
												<br>
											</div>
										</li>`;
					} else {
						...
						if(product.price_transfer){
							_content += `
								<li class="clearfix">
									<em>${product.quantity}x ${product.name}</em>
									<span>S/${product.price}</span>

									<div class="price_box  my-3" style="line-height: 9px;">
												<br>
								...
								<li class="clearfix">
									<em>${product.quantity}x ${product.name}</em>
									<span >S/${product.price}</span><span class="${product.active_old_regular}">${product.old_price_regular}</span></li> 
												`;	
										}
	...
	function get_cities_by_country(){
		...
		if (getElement(`select[name="country_id"]`).value != '') {
			axios.get(`/api/template_14/country/${getElement(`select[name="country_id"]`).value}/cities`)
				.then((response) => {
	...
	function get_provinces_by_city(){
		axios.get(`/api/template_14/city/${getElement(`select[name="city_id"]`).value}/provinces`)
			.then((response) => {
	...
	function get_districts_by_province(){
		axios.get(`/api/template_14/province/${$(`select[name="province_id"]`).val()}/districts`)
		.then((response) => {
	...
	$(`select[name="district_id"]`).on('change', function(E){
		...
		if (E.target.value != '') {
			axios.get(`/api/template_14/district/${E.target.value}/shipping-cost?cart=${localStorage.getItem('cart')}`)
				.then((response) => {
...
function culqi() {
		if (Culqi.token) { 
				...
				$.ajax({
						type: 'POST',
						url: '/api/template_14/order-confirm-payment',
  ...
  /*if( $('#select_payment_0').attr('checked') ) {
...
getElement(`#delivery-home`)
	.addEventListener('click', () => {
		...
		if ($(`select[name="district_id"]`).val() != "") {
			axios.get(`/api/template_14/district/${$(`select[name="district_id"]`).val()}/shipping-cost?cart=${localStorage.getItem('cart')}`)
				.then((response) => {
					let _max = Math.max.apply(null, response.data);
					max = _max;
					calculate_prices(_max);
...
getElement(`input[name="identity_document"]`)
	.addEventListener('keyup', function(e){
		...
		if (e.target.value.length >= 8) {
			axios.get(`/api/template_14/customer?identity_document=${e.target.value}`)
				.then((response) => {
					if (response.data) {
...
if (getElement(`#country_id`).value != '') {
	axios.get(`/api/template_14/country/${getElement(`#country_id`).value}/cities`)
		.then((response) => {
	...
	.then((city_id) => {
		axios.get(`/api/template_14/city/${city_id}/provinces`)
			.then((response) => {
	...
	.then(() => {
		axios.get(`/api/template_14/province/${getElement(`#province_id`).value}/districts`)
		.then((response) => {
...
$(`#with_invoice`)
    ...
    acount = _that.parent().next().children().find('.radio-btn-0').data("index");
    payment = _that.parent().next().children().find('.radio-btn-0').data("payment");
    //precio_transfer
    calculate_prices(max,payment);
 //alert(payment);

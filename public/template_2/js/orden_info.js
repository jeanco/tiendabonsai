// (() => {
	...
	function calculate_prices(shipping_cost = ''){
		axios.get(`/api/template_2/products/cart-lite?ids=${localStorage.getItem("cart")}`)
			.then((response) => {
				let _subTotal = 0,
					_content = ``,
					_discount = 0,
				...
				getElement(`#checkout_shipping-cost`).innerHTML = `S/${shipping_cost ? shipping_cost.toFixed(2) : 0.00}`;
	...
	getElement(`#checkout__save`)
		.addEventListener('click', function(e) {
			...
			if (getElement(`#other-billing-address`).checked && getElement(`select[name="district_id"]`).value == '') {
			...
			// } else {
				...
				let _otherBillingAddress = $('#other-billing-address').is(":checked");
				...
				$.ajax({
					...
					success: function(e){
						...
						if (e.payment_status) {
							...
							Culqi.publicKey = 'pk_live_3DMqdDzcpGoS106z';
							//Culqi.publicKey = 'pk_test_Gpa3RQJaG3d8C2wC';
					...
					error:function(jqXHR, textStatus, errorThrown)
					{
						...
						$.each(jqXHR.responseJSON, function (key, value) {
							if (key == "email") {
							  ...
							  $.each(value, function (errores, eror) {
							  	normalSwal(`Contraseña`, `Por su seguridad, su contraseña debe ser mayor a 7 caracteres.`, `info`);
	...
	if (getElement(`#country_id`).value != '') {
		axios.get(`/api/template_2/country/${getElement(`#country_id`).value}/cities`)
			.then((response) => {
		...
		.then((city_id) => {
			axios.get(`/api/template_2/city/${city_id}/provinces`)
				.then((response) => {
		...
		.then(() => {
			axios.get(`/api/template_2/province/${getElement(`#province_id`).value}/districts`)
			.then((response) => {
	...
	function get_cities_by_country(){
		axios.get(`/api/template_2/country/${getElement(`select[name="country_id"]`).value}/cities`)
			.then((response) => {
	...
	function get_provinces_by_city(){
		axios.get(`/api/template_2/city/${getElement(`select[name="city_id"]`).value}/provinces`)
			.then((response) => {
	...
	function get_districts_by_province(){
		axios.get(`/api/template_2/province/${$(`select[name="province_id"]`).val()}/districts`)
		.then((response) => {
	...
	$(`select[name="district_id"]`).on('change', function(E){
		...
		if (E.target.value != '') {
			axios.get(`/api/template_2/district/${E.target.value}/shipping-cost`)
				.then((response) => {
					calculate_prices(response.data);
					getElement(`#shipping-error`).innerHTML = ``;
				})
...
function culqi() {
		if (Culqi.token) { 
				...
				$.ajax({
						type: 'POST',
						url: '/api/template_2/order-confirm-payment',
...
getElement(`input[name="identity_document"]`)
	.addEventListener('keyup', function(e){
		...
		if (e.target.value.length >= 8) {
			axios.get(`/api/template_2/customer?identity_document=${e.target.value}`)
				.then((response) => {

(() => {
    ...
    axios.get(`/api/template_9/products/cart?ids=${localStorage.getItem("cart")}`)
        .then((response) => {
    ...
    $(`#apply_coupon`)
        .on('click', function(){
            ...
            axios.get(`/api/template_9/coupon/${_code}/information?ids=${localStorage.getItem("cart")}`)
                .then((response) => {
                    Swal.fire(
                    '',
    ...
    $(`select[name="place_id"]`).on('change', function(e){
        axios.post(`/api/template_9/set-place`, {

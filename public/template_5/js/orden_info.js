// (() => {
	...
	function calculate_prices(shipping_cost, credit_card_comission = false){
		...
		axios.get(`/api/template_5/products/cart-lite?ids=${localStorage.getItem("cart")}`)
			.then((response) => {
				let _subTotal = 0,
					_content = ``,
					_discount = 0,
	...
	function get_cities_by_country(){
		...
		if (getElement(`select[name="country_id"]`).value != '') {
			axios.get(`/api/template_5/country/${getElement(`select[name="country_id"]`).value}/cities`)
				.then((response) => {
	...
	function get_provinces_by_city(){
		axios.get(`/api/template_5/city/${getElement(`select[name="city_id"]`).value}/provinces`)
			.then((response) => {
	...
	function get_districts_by_province(){
		axios.get(`/api/template_5/province/${$(`select[name="province_id"]`).val()}/districts`)
		.then((response) => {
	...
	$(`select[name="district_id"]`).on('change', function(E){
		...
		if (E.target.value != '') {
			axios.get(`/api/template_5/district/${E.target.value}/shipping-cost?cart=${localStorage.getItem('cart')}`)
				.then((response) => {
...
function culqi() {
		if (Culqi.token) { 
				...
				$.ajax({
						type: 'POST',
						url: '/api/template_5/order-confirm-payment',
...
getElement(`input[name="identity_document"]`)
	.addEventListener('keyup', function(e){
		...
		if (e.target.value.length >= 8) {
			axios.get(`/api/template_5/customer?identity_document=${e.target.value}`)
				.then((response) => {
					if (response.data) {
...
if (getElement(`#country_id`).value != '') {
	axios.get(`/api/template_5/country/${getElement(`#country_id`).value}/cities`)
		.then((response) => {
	...
	.then((city_id) => {
		axios.get(`/api/template_5/city/${city_id}/provinces`)
			.then((response) => {
	...
	.then(() => {
		axios.get(`/api/template_5/province/${getElement(`#province_id`).value}/districts`)
		.then((response) => {
	...
	.then(() => {
		axios.get(`/api/template_5/district/${getElement(`#district_id`).value}/shipping-cost`)
			.then((response) => {
				calculate_prices(response.data);
				getElement(`#shipping-error`).innerHTML = ``;
			})

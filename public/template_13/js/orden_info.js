// (() => {
	...
	getElement(`#checkout__save`)
		.addEventListener('click', function(e) {
			...
			// } else {
				...
				// axios.post(`/api/template_13/order`, {
				// 	//'identity_document': getElement(`#checkout_identity-document`).value,
				// 	//'legal_name': $(`#checkout_legal-name`).val(),
	...
	function get_cities_by_country(){
		...
		if (getElement(`select[name="country_id"]`).value != '') {
			axios.get(`/api/template_13/country/${getElement(`select[name="country_id"]`).value}/cities`)
				.then((response) => {
	...
	function get_provinces_by_city(){
		axios.get(`/api/template_13/city/${getElement(`select[name="city_id"]`).value}/provinces`)
			.then((response) => {
	...
	function get_districts_by_province(){
		axios.get(`/api/template_13/province/${$(`select[name="province_id"]`).val()}/districts`)
		.then((response) => {
	...
	$(`select[name="district_id"]`).on('change', function(E){
		...
		if (E.target.value != '') {
			axios.get(`/api/template_13/district/${E.target.value}/shipping-cost?cart=${localStorage.getItem('cart')}`)
				.then((response) => {
...
function culqi() {
		if (Culqi.token) { 
				...
				$.ajax({
						type: 'POST',
						url: '/api/template_13/order-confirm-payment',
...
getElement(`#delivery-home`)
	.addEventListener('click', () => {
		...
		if ($(`select[name="district_id"]`).val() != "") {
			axios.get(`/api/template_13/district/${$(`select[name="district_id"]`).val()}/shipping-cost?cart=${localStorage.getItem('cart')}`)
				.then((response) => {
					let _max = Math.max.apply(null, response.data);
					calculate_prices(_max);
					getElement(`#shipping-error`).innerHTML = ``;
...
getElement(`input[name="identity_document"]`)
	.addEventListener('keyup', function(e){
		...
		if (e.target.value.length >= 8) {
			axios.get(`/api/template_13/customer?identity_document=${e.target.value}`)
				.then((response) => {
...
if (getElement(`#country_id`).value != '') {
	axios.get(`/api/template_13/country/${getElement(`#country_id`).value}/cities`)
		.then((response) => {
	...
	.then((city_id) => {
		axios.get(`/api/template_13/city/${city_id}/provinces`)
			.then((response) => {
	...
	.then(() => {
		axios.get(`/api/template_13/province/${getElement(`#province_id`).value}/districts`)
		.then((response) => {

(() => {
	...
	if (getElement(`#profile_country`).value != null) {
		axios.get(`/api/template_4/country/${getElement(`#profile_country`).value}/cities`)
			.then((response) => {
		...
		.then(() => {
			...
			axios.get(`/api/template_4/province/${getElement(`#profile_province-id`).value}/districts`)
			.then((response) => {

...
//alert(payment);
	...
	function calculate_prices(shipping_cost, payment_id = 2){
		axios.get(`/api/template_3/products/cart-lite?ids=${localStorage.getItem("cart")}&payment_id=${payment_id}`)
			.then((response) => {
				let _subTotal = 0,
					_content = ``,
					_discount = 0,
				...
				$.ajax({
					...
					success: function(e){
						...
						if (e.payment_status) {
							...
							//produccion
							Culqi.publicKey = 'pk_live_bcFPaq7mFU6QfLb0';
							Culqi.options({
								lang: 'es',
								modal: true,
					            installments: false,
	...
	function get_cities_by_country(){
		...
		if (getElement(`select[name="country_id"]`).value != '') {
			axios.get(`/api/template_3/country/${getElement(`select[name="country_id"]`).value}/cities`)
				.then((response) => {
	...
	$(`select[name="district_id"]`).on('change', function(E){
		...
		if (E.target.value != '') {
			axios.get(`/api/template_3/district/${E.target.value}/shipping-cost?cart=${localStorage.getItem('cart')}`)
				.then((response) => {
...
function culqi() {
		if (Culqi.token) { 
				...
				$.ajax({
						type: 'POST',
						url: '/api/template_3/order-confirm-payment',
...
getElement(`input[name="identity_document"]`)
	.addEventListener('keyup', function(e){
		...
		if (e.target.value.length >= 8) {
			axios.get(`/api/template_3/customer?identity_document=${e.target.value}`)
				.then((response) => {
					if (response.data) {
...
if (getElement(`#country_id`).value != '') {
	axios.get(`/api/template_3/country/${getElement(`#country_id`).value}/cities`)
		.then((response) => {
	...
	.then((city_id) => {
		axios.get(`/api/template_3/city/${city_id}/provinces`)
			.then((response) => {
	...
	.then(() => {
		axios.get(`/api/template_3/province/${getElement(`#province_id`).value}/districts`)
		.then((response) => {
